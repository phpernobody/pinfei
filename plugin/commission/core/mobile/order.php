...
class Order_EweiShopV2Page extends CommissionMobileLoginPage {
	...
	function main() {
		global $_W, $_GPC;
		if (empty($_GPC['downid'])) {
			$member = $this->model->getInfo($_W['openid'], array('total', 'ordercount0'));
		} else {
			$downOpenId = pdo_fetchall('select * from ' . tablename('ewei_shop_member') . ' where id=' . $_GPC['downid'])[0]['openid'];
			$member = $this->model->getInfo($downOpenId, array('total', 'ordercount0'));
		}
		include $this->template();
	}

	function get_down_list() {
		global $_W, $_GPC;

		$openid = pdo_fetchall('select * from ' . tablename('ewei_shop_member') . ' where id=' . $_GPC['downid'])[0]['openid'];
			...
			foreach ($list as &$row) {
				...
				if (!empty($this->set['openorderdetail'])) {
					$goods = pdo_fetchall("SELECT og.id,og.goodsid,g.thumb,og.price,og.total,g.title,og.optionname,"
							...
							. "og.content1,og.content2,og.content3,g.title,g.shorttitle from " . tablename('ewei_shop_order_goods') . " og"
					...
					foreach ($goods as &$g) {
						...
						$row['amount_total'] += $g['price'];
						$row['goods_number'] += $g['total'];
					}
					unset($g);

	...
	public function main()
	{
        ...
        foreach($list as $k => $v) {
            ...
            foreach($downList as $key => $value) {
                $downList[$key]['totalagent'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where hagentid=' . $downList[$key]['id'] . ' and isaagent=0');
                $downList[$k]['totalamout'] = intval(pdo_fetchcolumn('select sum(resultprice) from ' . tablename('ewei_shop_agent_order_finish') . ' where hagentid=' . $downList[$key]['id']));
            }

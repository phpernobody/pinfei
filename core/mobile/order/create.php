...
class Create_EweiShopV2Page extends MobileLoginPage
{
    ...
    public function main()
    {
        ...
        if (!($packageid)) {
            ...
            if (empty($id)) {
                ...
                if (empty($goods)) {
                    ...
                    foreach ($goods as $k => $v) {
                        ...
                        if (!(empty($opdata))) {
                             ...
                             * 此处修改 by yaowk
                             */
                            $goods[$k]['marketprice'] = m('order')->caculatePrice($member, $opdata);
                        }
                        else {
                 ...
                 * by yaowk
                 */
                $sql = 'SELECT provinceprice,cityprice,countyprice, id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,storeids,isverify,deduct,hasoption,preselltimeend,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
                $data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
                ...
                if (!(empty($optionid))) {
                    /**
                     * 增加provinceprice,cityprice,countyprice,
                     * by yaowk
                     */
                    $option = pdo_fetch('select provinceprice,cityprice,countyprice,id,title,marketprice,presellprice,goodssn,productsn,`virtual`,stock,weight,specs from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
                    if (!(empty($option))) {
                        $data['optionid'] = $optionid;
    ...
    public function submit()
    {
        ...
        foreach ($goods as $g) {
            ...
            if (!(empty($optionid))) {
                /**
                 * add provinceprice,cityprice,countyprice,
                 * by yaowk
                 */

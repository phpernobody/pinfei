...
class Notice_EweiShopV2Model
{
     ...
     */
    public function sendAbounsMessage($orderid = '0')
    {
        global $_W;
        if (empty($orderid))
        {
...
//            $sendOpenid = 'oCO-sv_eRVuy0_aHmizrfp-_L2js';
        }else {
...
//        $this->sendNotice(array('openid' => $sendOpenid, 'cusdefault' => $text));
//var_dump($text);exit;
        m('message')->sendTexts($sendOpenid, $text, $url, $account);
...
//        com_run('sms::callsms', array('tag' => 'willcancel', 'datas' => $datas, 'mobile' => $member['mobile']));
            ...
            if (empty($usernotice['saler_pay']))
            {
                ...
                if (!(empty($tm['openid'])))
                {
                    ...
                    foreach ($openids as $tmopenid )
                    {
                        ...
                        $this->sendNotice(array('openid' => $tmopenid, 'tag' => 'saler_pay', 'default' => $msg, 'cusdefault' => $text, 'datas' => $datas));
                    }
                }
            }
                    ...
                    if (empty($usernotice['saler_finish']))
                    {
                        ...
                        if (!(empty($tm['openid2'])))
                        {
                            ...
                            foreach ($openids as $tmopenid )
                            {
                                ...
                                $this->sendNotice(array('openid' => $tmopenid, 'tag' => 'saler_finish', 'cusdefault' => $text, 'default' => $msg, 'datas' => $datas));
                            }
                        }
                    }
    ...
    public function sendNotice(array $params)
    {
        ...
        {
//            var_dump($touser, $cusdefault_message, $account);exit;
            $ret = m('message')->sendTexts($touser, $cusdefault_message, '', $account);

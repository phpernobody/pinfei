...
//        var_dump($list);exit;
	...
	public function detail()
	{
		...
		if ($_W['ispost'])
		{
            ...
            // 存入会员的上级代理商id
            $hagentid = $_GPC['adata']['hagentid'];
            if (!empty($hagentid)) {
                $data['hagentid'] = intval($hagentid);
            } else {
                $data['hagentid'] = 0;
            }
...
//            $resss = $this->updateDownHagent($member['id']);
//            var_dump($resss);
//            exit;
        ...
        // 获取代理商信息
        $hagentid = $member['hagentid'];
        if (!empty($hagentid)) {
            $hagent = m('member')->getMember($member['hagentid']);
        } else {
    ...
    // 修改所有下级分销的代理关系
    public function updateDownHagent($memberId, $hagentId)
    {
        pdo_update('ewei_shop_member', array('hagentid' => $hagentId), array('id' => $memberId));

        $downList = pdo_fetchall('select * from ' . tablename('ewei_shop_member') . ' where agentid=' . $memberId);
        if (!empty($downList)) {
            foreach($downList as $k => $v) {
                $this->updateDownHagent($v['id'], $hagentId);
            }
        }
        return;
    }

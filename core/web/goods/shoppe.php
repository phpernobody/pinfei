...
class Shoppe_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		$shoppe = pdo_getall('ewei_shop_shoppe');
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_shoppe') . ' WHERE id = \'' . $id . '\' limit 1');
		}

		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'enabled' => intval($_GPC['enabled']), 'description' => $_GPC['description'], 'advimg' => save_media($_GPC['advimg']));

			if (!empty($id)) {
				pdo_update('ewei_shop_shoppe', $data, array('id' => $id));
				load()->func('file');
				file_delete($_GPC['thumb_old']);
				plog('shop.shoppe.edit', '修改专柜 ID: ' . $id);
			}
			 else {
				pdo_insert('ewei_shop_shoppe', $data);
				$id = pdo_insertid();
				plog('shop.shoppe.add', '添加专柜 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('goods/shoppe')));
		}
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id, name FROM ' . tablename('ewei_shop_shoppe') . ' WHERE id = \'' . $id . '\'');

		if (empty($item)) {
			$this->message('抱歉，分类不存在或是已经被删除！', webUrl('goods/shoppe', array('op' => 'display')), 'error');
		}


		pdo_delete('ewei_shop_shoppe', array('id' => $id), 'AND');
		plog('shop.shoppe.delete', '删除分类 ID: ' . $id . ' 专柜名称: ' . $item['name']);
		// m('shop')->getCategory(true);
	...
	public function enabled()
	{
		...
		pdo_update('ewei_shop_shoppe', array('enabled' => intval($_GPC['enabled'])), array('id' => $id));
